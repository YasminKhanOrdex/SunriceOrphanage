...
const Adoption: React.FC<{ navigation: any, route: any }> = ({ navigation, route }) => {
    ...
    const [mes, setMes] = useState("Your income is less than the child's Monthly expense, So You are not able to adopt this child")
    const OnSubmit = () => {
        if (name == '' ||
            age == '' ||
            mobile == '' ||
            ...
            (Platform.OS === 'ios') ? Alert.alert(mes) : Alert.alert("Sorry... ", Strings.lessIncome)
        }
        else {
           
            const data = { id:getID(),name: name, age: age, mobile: mobile, address: address, city: city, pinCode: pinCode, state: state, maritalStatus: maritalStatus, jobProfile: jobProfile, income: income }
            ...
            console.log("-----after pushing----", AdoptionData)
            Alert.alert("Thank You", Strings.AdoptionSucces)
